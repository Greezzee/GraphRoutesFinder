...
int main() {
	RouteSearchableGraph<> graph, graph2;
	...
	auto c1 = graph2.createNode(NodeType::SOURCE);
	auto c2 = graph2.createNode(NodeType::STANDART);
	auto c1_c2 = graph2.createLink(c1, c2, 1);

	auto converter = graph.mergeWith(graph2);
